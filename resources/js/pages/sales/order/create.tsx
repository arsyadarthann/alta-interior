...
export default function Create({
    ...
    // Track item and location selection for new items
    ...
    // Fetch sales order code when initialized
    useEffect(() => {
        if (initialized && auth?.user?.branch_id) {
            fetchSalesOrderCode(auth.user.branch_id);
        }
    }, [initialized, auth?.user?.branch_id]);
    ...
    const fetchSalesOrderCode = useCallback(
        (branchId: number) => {
            if (isFetchingCode.current) return;

            isFetchingCode.current = true;
            fetch(
                route('sales.order.getCode', {
                    source_able_id: branchId,
                ...
                .catch(() => {
                    showErrorToast(['Failed to get sales order code']);
                })
                .finally(() => {
        ...
        (customerId: number) => {
            ...
            fetch(
                ...
                .then((responseData) => {
                    ...
                    // Update pricing for any existing items in the order
    ...
    // Fetch items when component is mounted (only once)
    useEffect(() => {
    ...
    // Calculate totals whenever sales order details change
    useEffect(() => {
    ...
    const handleItemSelection = (itemId: number, isAddingNew: boolean, index: number = -1) => {
        ...
        if (isAddingNew) {
            // Update form state to track selected item
            setFormState((prev) => ({
                ...prev,
                selectedItem: selectedItem,
            }));

            // Keep current location if already selected
            setData({
                ...data,
                new_item: {
                    ...data.new_item,
            ...
            // If location is already selected, fetch stock
            ...
            // Keep existing source if already selected
            updatedItems[index] = {
                ...currentItem,
                item_id: itemId,
                unit_price: unitPrice,
            ...
            // If source is already selected, fetch stock
    ...
    const handleLocationSelection = (locationValue: string, isAddingNew: boolean, index: number = -1) => {
        ...
        if (isAddingNew) {
            // Update form state to track selected location
            setFormState((prev) => ({
                ...prev,
    ...
    const handleCustomerChange = (value: string) => {
        ...
        if (value === 'new') {
            setData({
                ...
                customer_id: null,
                customer_name: '',
                sales_order_details: [], // Reset item list
            });
        } else if (value) {
    ...
    // Update the setEditingIndex function to fetch stock when editing
    const startEditing = (index: number) => {
        ...
        if (orderItem.item_id && orderItem.item_source_able_id && orderItem.item_source_able_type) {
            // Fetch the current stock for this item/location combination
            fetchItemStock(orderItem.item_id, orderItem.item_source_able_type, orderItem.item_source_able_id).then((stockQty) => {
                // Update the available stock for this index
                const newAvailableStocks = { ...availableStocks };
                newAvailableStocks[index] = stockQty;
                setAvailableStocks(newAvailableStocks);

                // Now set the editing index after we've fetched the stock
                setEditingIndex(index);
            });
        } else {
            // If for some reason the item doesn't have all the necessary data,
            // just set the editing index directly
            setEditingIndex(index);
        }
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="rounded-lg bg-white px-8 py-6">
                <div className="mb-6 flex items-center justify-between">
                    <Heading title="Create Sales Order" description="Create a new sales order for your customers." />
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {!addingItem && editingIndex === null && (
                                            <Button
                                                ...
                                                className="mt-2"
                                                disabled={!items.length || (!data.customer_id && data.customer_name === '')}
                                            >
