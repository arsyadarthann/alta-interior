...
export default function ExpenseEdit({ expense, branches, warehouses }: Props) {
    ...
    const saveExpenseItem = () => {
        if (editingIndex !== null) {
            // After editing an existing item, recalculate the total
            const updatedTotal = data.expense_details.reduce((sum, item) => sum + Number(item.amount), 0);
            setData((prev) => ({
                ...prev,
            ...
            if (data.new_item.name && data.new_item.amount > 0) {
                ...
                const newTotal = updatedDetails.reduce((sum, item) => sum + Number(item.amount), 0);
                setData((prev) => ({
                    ...prev,
    ...
    const updateExpenseItem = (index: number, field: 'name' | 'amount', value: string | number) => {
        ...
        // Always recalculate the total when updating an item
        const newTotal = updatedItems.reduce((sum, item) => sum + Number(item.amount), 0);
                ...
                <div className="relative grid min-w-[200px] flex-1 gap-2">
                    ...
                    <div className="relative">
                        <Input
                            ...
                            type="number"
                            min="0"
                            value={item.amount === 0 ? '' : formatDecimal(item.amount)}
                            onChange={(e) => {
                                if (isAddingNew) {
                                    setData('new_item', {
                                        ...data.new_item,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="rounded-lg bg-white px-8 py-6">
                <div className="mb-6 flex items-center justify-between">
                    <Heading title="Edit Expense" description="Edit an existing expense record." />
