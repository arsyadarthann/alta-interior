    ...
    return {
        ...
        cell: ({ row }) => {
            ...
            const actionItems = actions(data)
                .filter((action) => !action.permission || (hasPermission && hasPermission(action.permission)))
            ...
            if (isHighlighted && isHighlighted(data)) {
                return (
                    <span className="inline-flex items-center gap-1 rounded-md border border-blue-100 bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700">
                        <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-blue-500 shadow-[0_0_2px_#3b82f6]"></span>
                        Editing
                    </span>
                );
            }
                ...
                <>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="h-8 w-8 p-0 hover:bg-transparent focus:ring-0 focus:outline-none focus-visible:ring-0">
                    ...
                    {selectedAction?.showConfirmDialog && (
                        <DeleteConfirmationDialog
                            ...
                            title={
                                typeof selectedAction.confirmDialogProps === 'function'
